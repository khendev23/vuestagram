<template>
  <div class="post" v-for="(vuestaData, i) in $store.state.vuestaData" :key="i">
    <div class="post-header">
      <div class="profile" :style="{ backgroundImage : `url(${vuestaData.userImage})` }"></div>
      <span class="profile-name">{{vuestaData.name}}</span>
    </div>
    <div @click="$store.commit('plusLike', i)" :class="selectedFilter" class="post-body" :style="{ backgroundImage : `url(${vuestaData.postImage})` }"></div>
